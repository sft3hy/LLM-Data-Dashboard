...
def add_message():
    if st.session_state.input_message.strip():
        ...
        new_code = code_refiner(\"\"\"{existing_code}\"\"\", st.session_state.input_message)
        exec(new_code, {empty})
...
existing = """from streamlit_folium import folium_static
import streamlit as st
import pandas as pd
import json
...
output_refined_dashboard(existing)
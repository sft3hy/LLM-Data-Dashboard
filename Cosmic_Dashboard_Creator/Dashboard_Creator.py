...
from utils.misc import (
    ...
    clean_set_page_config,
    get_new_filename,
    choose_text_generator,
    fix_json,
    correct_code_remotely,
    wait_for_file,
    generate_pages,
    add_python_comments,
)
...
with top_container:
    ...
    with col2:
        ...
        st.markdown(
            "LLM Temperature",
            help="Set the model temperature. 0 = more logical, 0.4 = more creative",
        )
...
with bottom_container:
    ...
    with col2:
        uploaded_files = st.file_uploader(
            ...
            type=[
                ...
                "csv",
                "zip",
            ],  # Shapefiles are usually uploaded as zipped archives
    ...
    file_preview = st.checkbox("Show file information")
...
if user_input:
    if not selected_files:
        st.error(
            "You must either upload new files or select existing files before creating a dashboard!"
        )
...
if selected_files:
    ...
    summarization_methods = [
        {
            "label": "llm",
            "description": "Uses the LLM to parse your data and pass enriched information to the goal generator",
        },
        {
            "label": "default",
        ...
        {
            "label": "columns",
            "description": "Passes the dataset column names to the goal generator",
        },
    ]
            ...
            for selected_file in selected_files:
                with st.spinner(text="Summarizing data set..."):
                    summary = lida.summarize(
                        selected_file,
                        summary_method=(
                            selected_method if selected_method else "columns"
                        ),
...
if (
    ...
    with st.spinner("Generating Dashboard..."):
        ...
        try:
            ...
            maybe_correct = correct_code_remotely(
                code_snippet=response, extra_context=formatted, file_name=""
            )
            whole_code = clean_set_page_config(
                f"{commonly_missed_imports}{maybe_correct}"
            )
            ...
            gimme_more = output_refined_dashboard(
            ...
            add_user_message(
                only_filename, message_contents=user_input, user_id=optional_creator
            )
    ...
    with st.spinner("Writing file..."):
        if wait_for_file(filename_to_write):
            ...
            lida = st.Page("Cosmic_Dashboard_Creator/LIDA.py", title="LIDA", icon="📊")

            pg = st.navigation(
                {
                    "Cosmic Dashboard Creator": [
                        creator,
                        about,
                        model_info,
                        kaggle,

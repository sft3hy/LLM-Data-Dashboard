...
if view_old and not hide_old:
    for message in previous_messages:
        if message["role"] == "user":
            st.chat_message("user", avatar=person_svg).write(
            ...
            assistant_message.expander(message["assistant_code_expander"]).code(
                message["assistant_code_top"] + message["assistant_code"],
                language="python",
            )
            st.divider()
...
def update_sections(user_message, generated_code, bot_response, code_expander_text):
    ...
    with code_explanation_placeholder.container():
        ...
        assistant_message.expander(code_expander_text).code(
...
if not view_old:
    update_sections(
        prev_user_message["message_contents"],
        prev_bot_message["assistant_code"],
        prev_bot_message["message_contents"],
        prev_bot_message["assistant_code_expander"],
    )

# Add the chat input field
input_message = st.chat_input(
    placeholder="Show cumulative totals instead of daily changes"
)
...
These are the file path(s): ['user_uploaded_files/healthcare-dataset-stroke-data.csv']"""
    refined_code = code_refiner(
        previous_code, f"{input_message} {context}", model=selected_model
    )
    ...
    # Save messages
    add_user_message(
        file_path=FILENAME, message_contents=input_message, user_id=user_email
    )
